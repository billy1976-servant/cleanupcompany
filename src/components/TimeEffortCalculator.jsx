// === TIME EFFORT CALCULATOR COMPONENT ===
  ...
  return (
    <div className="mt-8 bg-white p-8 rounded-2xl shadow-2xl border-t-4 border-blue-500">
      <h2 className="text-3xl font-bold text-center text-gray-900 mb-8">
        {COPY.calculator.timeEffort.title}
      </h2>
      
      <div className="space-y-8 my-10">
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="font-bold text-gray-700">{COPY.calculator.timeEffort.crewSize}</label>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="font-bold text-gray-700">{COPY.calculator.timeEffort.cleanupTime}</label>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="font-bold text-gray-700">{COPY.calculator.timeEffort.workDays}</label>
      ...
      <div className="text-center bg-blue-50 p-6 rounded-xl border border-blue-200">
        <p className="text-lg text-gray-700">{COPY.calculator.timeEffort.result}</p>
        <p className="text-5xl font-extrabold text-blue-600 my-2">{totalHoursLost}</p>
        <p className="text-lg text-gray-700">{COPY.calculator.timeEffort.resultSubtext}</p>
        <p className="text-sm text-gray-600 mt-2">
          {COPY.calculator.timeEffort.resultDetail.replace('{hours}', formatHours(productiveHoursGained))}
        </p>
      </div>
      ...
      <div className="text-center mt-10">
        ...
        >
          {COPY.calculator.timeEffort.nextButton}
        </button>
      </div>
    </div>
  );
...
export default TimeEffortCalculator;
